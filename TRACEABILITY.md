...
> **Fase actual:** Bloque 1 (Foundation) cerrado el 2026-09-23. Los estados reflejan evidencia real: `VALIDATED` solo con tests ejecutados y en verde en el gate del Bloque 1; `PARTIAL` para requisitos transversales o compartidos con bloques posteriores. Los requisitos de los bloques 2–6 siguen en `NOT_IMPLEMENTED`.
...
| GOV-007 | §3 | Dependency injection y Protocol/ABC donde aporte valor | todo el paquete | Protocols `DataSource`, `ExpectedReturnProvider`, `CovarianceEstimator`; dependencias por constructor | N/A-Infra | — | Tests inyectan configuraciones y fuentes alternativas | 1–6 | PARTIAL (transversal hasta B6) |
...
| RequirementID | § | Descripción | Módulo responsable | Clase/función prevista | Tipo matemático | Dependencias | Test de aceptación | Bloque | Estado |
|---|---|---|---|---|---|---|---|---|---|
| CFG-001 | §4 | `EngineConfig` raíz inmutable que agrega subconfiguraciones | `config/engine_config.py` | `EngineConfig` | N/A-Infra | — | `tests/unit/config/test_config_loader.py` | 1 | VALIDATED |
...
| CFG-003 | §4 | `ReturnConfig` (tipo de retorno, TradingDays, justificación log, rf, modo expected return) | `config/return_config.py` | `ReturnConfig` | N/A-Infra | — | `tests/unit/config/test_config_loader.py` | 1 | VALIDATED |
| CFG-004 | §4 | `RiskConfig` (método de covarianza, tolerancia PSD, método de reparación, floor) | `config/risk_config.py` | `RiskConfig` | N/A-Infra | — | `tests/unit/config/test_config_loader.py` | 1 | VALIDATED |
| CFG-005 | §4 | `ConstraintConfig` (LongOnly, límites globales, grupos, turnover, `restricted_existing_position_policy` (E-09), caja) | `config/constraint_config.py` | `ConstraintConfig` (long_only, límites globales, política de restringidos) | N/A-Infra | A-09, A-12, A-30 | `tests/unit/config/test_config_loader.py` | 1 (base), 2–4 (extensiones) | PARTIAL (base B1; extensiones B2–B4) |
| CFG-006 | §4 | `TransactionCostConfig` (unidad de entrada, precedencia de fuentes). El horizonte NO reside aquí: se toma de `OptimizationHorizonYears` (CFG-018) | `config/transaction_cost_config.py` | `TransactionCostConfig` | N/A-Infra | A-04, CFG-018 | `tests/unit/config/test_config_loader.py`, `tests/unit/costs/test_transaction_cost_inputs.py` | 1 | VALIDATED |
...
| CFG-016 | Bloque 1 | Única fuente de verdad por propiedad; sin configuración duplicada; validación de coherencia cruzada | `config/loader.py` | `load_engine_config` (claves desconocidas/ausentes, tipos, coherencia) | N/A-Infra | — | `tests/unit/config/test_config_loader.py` | 1 | VALIDATED |
| CFG-017 | §31, §70 | `ConfigSnapshot` canónico y `ConfigHash` determinista | `config/hashing.py` | `config_snapshot`, `config_hash` | N/A-Infra | — | `tests/unit/config/test_config_hashing.py` | 1 | VALIDATED |
...
| DAT-004 | §5.1 | Fuente DataFrame | `data/sources/dataframe_source.py` | `DataFrameSource` | N/A-Datos | DAT-003 | `tests/unit/data/test_sources.py` | 1 | VALIDATED |
| DAT-005 | §5.1 | Fuente Parquet | `data/sources/file_sources.py` | `ParquetSource` | N/A-Datos | DAT-003 | `tests/unit/data/test_sources.py` | 1 | VALIDATED |
| DAT-006 | §5.1 | Fuente CSV | `data/sources/file_sources.py` | `CSVSource` (parseo numérico exacto) | N/A-Datos | DAT-003 | `tests/unit/data/test_sources.py` | 1 | VALIDATED |
...
| DAT-009 | §6 | Esquema de universo (18 campos: AssetID … RestrictedAssetFlag) | `models/asset.py`, `data/validation/universe_validator.py` | `AssetMetadata`, `Universe`, `UniverseValidator` | N/A-Datos | A-31 | `tests/unit/data/test_universe_validation.py` | 1 | VALIDATED |
| DAT-010 | §7 | Esquema de carteras actuales (PortfolioID, AssetID/Ticker, CurrentWeight) | `data/validation/portfolio_validator.py` | `PortfolioValidator` | N/A-Datos | — | `tests/unit/data/test_portfolio_validation.py` | 1 | VALIDATED |
...
| DAT-012 | §7 | Preservar explícitamente la composición actual (turnover, liquidaciones, nuevos/eliminados, distancia) | `models/portfolio.py` | `CurrentPortfolioState`, `CurrentPortfolioComposition` | N/A-Datos | — | `tests/unit/data/test_current_portfolio_state.py` | 1 | VALIDATED |
...
| DAT-015 | §6.5 ARCH | Entradas opcionales: ExternalAlpha, ReferenceBenchmarkWeights, AssetBetas, FactorExposures, SimulatedReturns | `models/risk_model.py`, `data/validation/alpha_validator.py` | `ExternalAlpha`, `build_external_alpha` | N/A-Datos | A-10, A-35 | `tests/unit/returns/test_expected_returns.py` | 1 (alpha), 4 (resto) | PARTIAL (alpha en B1; benchmark/betas/factores/simulaciones en B4) |
| DAT-016 | §31, §62 (E-06) | `CurrentPortfolioStateHash` determinista a partir de `(AssetID, CurrentWeight)`: pares ordenados por AssetID, pesos serializados exactamente (`float.hex`, `-0.0`→`0.0`, sin redondeo), filas de peso exactamente cero excluidas, SHA-256 | `models/portfolio.py` | `current_portfolio_state_hash`, `CurrentPortfolioState.state_hash` | N/A-Infra | A-06, DAT-012 | `tests/unit/data/test_current_portfolio_state.py`, `tests/property/test_properties.py::test_state_hash_*` | 1 | VALIDATED |
...
| DAT-022 | §10 | Precios ≤ 0 | `data/validation/market_data_validator.py` | `NON_POSITIVE_PRICE` | N/A-Datos | — | `tests/unit/data/test_market_data_validation.py::test_bad_prices_are_detected_per_asset` | 1 | VALIDATED |
| DAT-023 | §10 | NaN (missing data) sin relleno silencioso | `data/validation/market_data_validator.py` | `MISSING_PRICE`, `MISSING_OBSERVATION` | N/A-Datos | — | `tests/unit/data/test_market_data_validation.py::test_bad_prices_are_detected_per_asset`, `tests/unit/returns/test_returns_engine.py::test_missing_cells_are_not_filled` | 1 | VALIDATED |
| DAT-024 | §10 | Valores inf | `data/validation/*` | `NON_FINITE_VALUE` | N/A-Datos | — | `tests/unit/data/test_market_data_validation.py::test_bad_prices_are_detected_per_asset` | 1 | VALIDATED |
...
| DAT-028 | §10 | Metadatos incompletos (campos obligatorios/condicionales del universo) | `data/validation/universe_validator.py` | `MISSING_METADATA`, `MISSING_RECOMMENDED_METADATA` | N/A-Datos | A-04, A-10 | `tests/unit/data/test_universe_validation.py` | 1 | PARTIAL (obligatorios y recomendados en B1; campos condicionales a restricciones de grupo en B2) |
| DAT-029 | §10 | Pesos inconsistentes (suma ≠ 1 fuera de tolerancia, negativos con LongOnly, activo desconocido) | `data/validation/portfolio_validator.py` | `_checked_weight`, `_apply_residual_policy` | N/A-Datos | A-34 | `tests/unit/data/test_portfolio_validation.py` | 1 | VALIDATED |
...
| RET-002 | §8 | EXTERNAL_ALPHA: ingesta, alineación por AssetID, conversión de horizonte declarado a base anualizada (compatible con `OptimizationHorizonYears`, E-03), validación | `returns/expected/external_alpha.py`, `data/validation/alpha_validator.py` | `ExternalAlphaProvider`, `build_external_alpha` | N/A-Datos | DAT-015, CFG-018 | `tests/unit/returns/test_expected_returns.py` | 1 | VALIDATED |
...
| RSK-013 | §11, §31 | `RiskModel` global (mu, Σ read-only) con `MuSigmaVersion` determinista | `risk/risk_model_builder.py`, `models/risk_model.py` | `CovarianceBuilder`, `build_risk_model`, `RiskModel.mu_sigma_version` | N/A-Infra | RSK-002..012, RET-* | `tests/unit/risk/test_risk_model.py` | 1 | VALIDATED |
...
| TC-005 | §6, §23, Bloque 1 | Modelo de datos de costes: fuentes, precedencia, unidades, validación (finitos, ≥ 0) | `models/costs.py`, `data/validation/transaction_cost_inputs.py` | `AssetCostVector`, `resolve_asset_costs`, `CostResolution` | N/A-Datos | A-04 | `tests/unit/costs/test_transaction_cost_inputs.py` | 1 | VALIDATED |
...
| TST-018 | Bloque 1 | Tests obligatorios B1: returns, annualization, missing data, duplicados, PSD, near singular, singular, PSD repair, config immutability, transaction-cost input validation | `tests/unit/` | tests obligatorios del Bloque 1 | N/A-Infra | — | Todos presentes y en verde (ver CHANGELOG, gate B1) | 1 | VALIDATED |
