...
> **Fase actual:** Bloque 1 (Foundation) cerrado el 2026-09-23 y auditado el mismo día
> (`AUDIT_BLOCK_1.md`, `AUDIT_STATUS = PASS_WITH_CHANGES`). Los estados reflejan evidencia real:
> `VALIDATED` solo con tests ejecutados y en verde en el gate del Bloque 1; `PARTIAL` para
> requisitos transversales o compartidos con bloques posteriores. Los requisitos de los bloques
> 2–6 siguen en `NOT_IMPLEMENTED`. El cierre de auditoría amplió la evidencia de los requisitos
> que solo citaban un fichero de test (ahora citan la función concreta) y corrigió la evidencia
> circular de TST-018; ningún estado cambió como consecuencia del cierre — ver `CHANGELOG.md`.
...
| Tests e invariantes | TST | §72–76 | 22 | 19 | 2 | 0 | 1 |
| Visualización / principio final | VIS | §1, §82 | 3 | 3 | 0 | 0 | 0 |
| **Total** | | | **316** | **245** | **13** | **0** | **58** |
...
| GOV-007 | §3 | Dependency injection y Protocol/ABC donde aporte valor | todo el paquete | Protocols `DataSource` (`data/sources/base.py`), `ExpectedReturnProvider` (`returns/expected/base.py`), `CovarianceEstimator` (`risk/covariance/base.py`); dependencias por constructor | N/A-Infra | — | `tests/unit/data/test_sources.py::test_all_sources_yield_identical_canonical_tables` (dos implementaciones de `DataSource` inyectadas de forma intercambiable), `tests/unit/returns/test_expected_returns.py::test_factory_follows_configuration` (`ExpectedReturnProvider` seleccionado por config, no acoplado), `tests/unit/risk/test_covariance_estimators.py::test_factory_follows_configuration` (`CovarianceEstimator` inyectado vía factoría) | 1–6 | PARTIAL (transversal hasta B6; Bloque 1 cubre `DataSource`/`ExpectedReturnProvider`/`CovarianceEstimator`, quedan `OptimizationBackend` y el resto de Protocols de bloques posteriores) |
...
| GOV-011 | §80 | No regresión: baseline antes, suite completa después, comparar | proceso | baseline + suite completa | N/A-Infra | — | B1: baseline inicial (sin tests previos); suite final 252 passed | 1–6 | PARTIAL (transversal hasta B6) |
...
| CFG-005 | §4 | `ConstraintConfig` (LongOnly, límites globales, grupos, turnover, `restricted_existing_position_policy` (E-09), caja) | `config/constraint_config.py` | `ConstraintConfig` (long_only, límites globales, política de restringidos) | N/A-Infra | A-09, A-12, A-30 | `tests/unit/config/test_config_loader.py::test_types_and_enum_names_are_checked`, `tests/unit/data/test_restricted_policy.py::test_policy_may_be_omitted_in_config` | 1 (base), 2–4 (extensiones) | PARTIAL (base B1; extensiones B2–B4) |
...
| CFG-017 | §31, §70 | `ConfigSnapshot` canónico y `ConfigHash` determinista | `config/hashing.py` | `config_snapshot`, `config_hash` | N/A-Infra | — | `tests/unit/config/test_config_hashing.py::test_hash_independent_of_key_order`, `::test_hash_is_stable_value`, `::test_any_parameter_change_changes_hash`, `::test_snapshot_is_complete_canonical_json` | 1 | VALIDATED |
...
| DAT-013 | §59, Bloque 3 | Índice de activos global ↔ elegible ↔ local de composición | `models/universe_index.py` | `AssetIndex` (nivel global) | N/A-Datos | A-31 | `tests/unit/data/test_current_portfolio_state.py::test_asset_index_round_trip` | 1 (global), 3 (tres niveles) | PARTIAL (nivel global B1; eligible/local en B3) |
...
| DAT-028 | §10 | Metadatos incompletos (campos obligatorios/condicionales del universo) | `data/validation/universe_validator.py` | `MISSING_METADATA`, `MISSING_RECOMMENDED_METADATA` | N/A-Datos | A-04, A-10 | `tests/unit/data/test_universe_validation.py::test_null_required_metadata_is_error`, `::test_missing_recommended_metadata_is_warning` | 1 | PARTIAL (obligatorios y recomendados en B1; campos condicionales a restricciones de grupo en B2) |
