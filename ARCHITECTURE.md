...
### 3.4 Desviaciones y decisiones del Bloque 3 respecto al diseño original

Registradas al implementar el Bloque 3 (`CandidateEngine`, sustituciones y `GLOBAL_CANDIDATE_FRONTIER`).
Ninguna sustituye Beam por Greedy, Global Frontier por una sola composición, Net por Gross − costes,
SOCP por un QP incompatible ni MIQP por una heurística: la cardinalidad se garantiza en la generación
discreta y se documenta como **no equivalente** a una formulación MIQP exacta (`CON-019`, `PARTIAL`).

| # | Diseño original | Implementación real (Bloque 3) | Racional |
|---|---|---|---|
| 1 | `candidates/` con `eligibility`, `screening`, `exploration`, `swap_generator`, `local_search`, `beam_search`, `tabu`, `composition_hash`, `candidate_engine`, `diagnostics` | Igual salvo `tabu.py` (**no implementado**: opcional, §30, `CAN-014` sigue `NOT_IMPLEMENTED`) y cuatro módulos añadidos: `universe_data.py` (`UniverseSnapshot`: arrays alineados con el índice global, sin copiar `Σ`), `prepared.py` (`CompositionFactory`/`PreparedComposition`: restricciones del Bloque 2 + `Σ_C` de una composición), `evaluation.py` (`CompositionEvaluator`, `ProjectedWeightsEvaluator`) y `expansion.py` (`NeighborhoodExpander`: primitiva compartida por haz y búsqueda local) | Evita duplicar el vecindario entre `BeamSearch` y `LocalSearch` y reutiliza el `ConstraintCompiler`/`PreFeasibilityChecker`/`SolutionValidator` validados en lugar de reimplementarlos |
| 2 | `engine.py` (orquestador `PortfolioEngine`) como módulo permitido del B3 | **No creado.** El pipeline `FAST_PRODUCTION` secuencial es `frontiers/global_frontier.py::GlobalCandidateFrontierEngine` (candidatos → frontera continua por composición → Pareto global) | El orquestador de alto nivel (datos → resultados) es integración de B6; `OPT-002` queda `PARTIAL` por este motivo |
| 3 | `candidates` depende de `optimizers` para evaluar composiciones con QP pequeños (§4.1, §10.3) | `candidates` **no** depende de `optimizers` ni de `frontiers`: el modo exacto `QP_UTILITY` es `frontiers/candidate_evaluator.py::QPCompositionEvaluator`, inyectado en el `CandidateEngine` (`GlobalCandidateFrontierEngine` lo construye si `evaluation_mode = QP_UTILITY`) | Mantiene el grafo acíclico y la búsqueda independiente de los backends; verificado por `tests/unit/test_architecture_rules.py::test_candidate_engine_is_independent_of_solvers_and_frontiers` |
| 4 | Un único criterio de utilidad para la búsqueda | **Un perfil de aversión al riesgo `λ` por región de la frontera** (`CandidateConfig.utility_risk_aversions`): la búsqueda en haz se ejecuta por perfil y el cupo `max_candidates` se reparte por rango entre perfiles | Una composición óptima para un solo `λ` cubre solo una zona de la frontera; la frontera global necesita composiciones que sean eficientes en zonas distintas. Los perfiles viven en la configuración |
| 5 | Evaluación "analítica o QP pequeño" (§10.3) | `PROJECTED_WEIGHTS`: pesos actuales transferidos, proyectados exactamente sobre cotas y presupuesto y refinados con gradiente proyectado; **cota inferior** de la utilidad óptima, con las restricciones de grupo/turnover comprobadas después (`CompositionEstimate.violations`). `QP_UTILITY`: óptimo exacto con `FrontierSession` del B2 | La proyección es rápida y nunca sobrestima; se contrasta con SLSQP independiente en `tests/unit/candidates/test_evaluation.py`. Los `estimated_*` son estimaciones, no resultados de la optimización |
| 6 | `CandidateScore` único | `candidate_score` (prior heurístico de screening acumulado por los movimientos) y `estimated_utility_gain` (utilidad de media-varianza neta de costes en el perfil de origen) son magnitudes distintas; ninguna es el objetivo optimizado del B2 | Evita presentar un score heurístico como solución del problema continuo |
| 7 | `EngineConfig.candidates: CandidateConfig \| None` (§8.1) | `EngineConfig.candidates: CandidateConfig` **obligatorio**, sin defaults en código (mismo criterio que `frontier`/`solver`/`benchmark` en el B2); `load_engine_config` admite subtablas TOML anidadas | Cada parámetro lo consume código real (test `test_every_candidate_parameter_is_consumed_by_the_engine_code`) |
| 8 | Módulos de salida `outputs/schemas.py` y `outputs/builders.py` ampliados | Módulos nuevos `outputs/candidate_schemas.py` y `outputs/candidate_builders.py` (filas de `CandidateCompositions`, `CandidateDiagnostics`, puntos globales y `visualization_dataset`) | No se tocan los módulos validados del Bloque 2 |
| 9 | `parallel/` solo en el Bloque 5 | Se crea `parallel/determinism.py` (`derive_seed`, `sequence_key`, `tie_break_key`) porque el plan lo asigna al B3 (`PAR-009`, `PAR-010`, `REP-003`); no hay multiprocessing ni memoria compartida (`test_no_later_block_packages_exist` prohíbe el resto de `parallel/`) | Semillas y orden estable no dependen de `hash()` de Python |
| 10 | Índices global/elegible/local | `models/universe_index.py`: `AssetIndex` (global), `EligibleUniverseIndex` (subconjunto no consecutivo) y `CompositionIndexMap` (local ↔ global ↔ elegible; `-1` si un activo actual no comprable no es elegible). La cartera se mapea por `AssetID`, nunca por `CurrentPortfolioState.global_indices` (que usa el índice del universo, no el del modelo de riesgo) | Evita interpretar un índice de la covarianza global como índice local |
| 11 | `CompositionID` y `CompositionHash` campos distintos | Misma cadena SHA-256 (el `CompositionID` del B2 ya era el hash de los AssetID ordenados); `CandidateComposition.composition_hash` es una propiedad | La frontera continua del B2 y el candidato identifican la misma composición con el mismo valor |
| 12 | `LiquidityConstraint` de A-11 | **Implementada en la remediación** (E-11): `constraints/liquidity.py` (`liquidity_capacity`, única implementación de la fórmula); `LiquidityConfig`/`FxRate` en `config/constraint_config.py`; `AssetMetadata.adv_currency` y `PortfolioSpec.nav_currency` (columnas `ADVCurrency`/`NAVCurrency`); `ConstraintCompiler._bounds` emite `LiquidityCapRule` y fija `w ≤ max(w_current, capacidad)`; `PreFeasibilityChecker.check_liquidity` (`LIQUIDITY_CAP_BELOW_MIN_WEIGHT`, `LIQUIDITY_CAPS_BELOW_BUDGET`); `SolutionValidator._liquidity` (desde la capacidad y `w_current` de la regla); `CandidateEngine` valida los datos al iniciar, acota el peso de entrada del screening y la cardinalidad y anota `A11_ADV_NAV_LIQUIDITY_CAPS_ACTIVE` con las posiciones protegidas por encima de la capacidad. `CON-012` y `FEA-006` `VALIDATED` tras el cierre del contrato de datos (`AdvUnit`, `ADVSource`, divisas obligatorias, datos también para restringidos; `constraints/liquidity.py::check_adv_unit`/`check_currencies`; `SolutionValidator` comprueba de nuevo el contrato de datos desde la `LiquidityCapRule`) | Decisión funcional del usuario: posición existente protegida, configuración sin valores por defecto, misma divisa o FX explícito |
| 13 | Política de activos actuales no elegibles | **E-10 (Existing Non-Buyable Position Policy, `MASTER_SPEC` §12 y Anexo A):** un activo mantenido no comprable (`EligibleFlag` falso, `LiquidityFlag` falso o desconocido con política conservadora, o fuera del `InvestmentUniverse`) y no restringido cumple `0 ≤ w ≤ min(w_current, MaxWeight)`; si no está en cartera, `w = 0`. Predicado único en `models/purchasability.py` (`purchase_block_reasons`); `ConstraintSet` lleva la política de banderas desconocidas; `ConstraintCompiler._bounds` fija las cotas y emite `NonBuyablePositionRule`; `PreFeasibilityChecker._non_buyable` diagnostica los topes que impiden el presupuesto; `SolutionValidator._non_buyable` lo comprueba desde `w_current` (independiente de las cotas compiladas); `EligibilityResult.weight_cap` limita el peso de entrada que el screening supone a un activo `LIQUIDATE_ONLY` que reentra en una composición. La precedencia de E-09 se conserva | Sustituye la «limitación conocida» del cierre del B3 (`AUDIT_BLOCK_3.md` H-1): el frontera continua, las estimaciones de candidatos y la frontera global aceptaban subir un activo no comprable de 10 % a 100 % y lo validaban |
| 14 | Presupuesto de evaluaciones | `CandidateConfig.max_evaluations` es **por perfil λ y por fase** (haz y pulido); tope total `len(utility_risk_aversions) × 2 × max_evaluations` (más la raíz de cada perfil) | Es el comportamiento implementado desde el B3 (`CHANGELOG.md`: el haz agotaba el presupuesto y la búsqueda local nunca se ejecutaba); los documentos de gobierno no lo definían y se ha alineado la documentación del código, no el significado (`REMEDIATION_BLOCK_3.md`) |
| 15 | Rendimiento de la generación de candidatos | `MemoizedEvaluator` (memoria local de `(composición, λ, utilidad de referencia)` dentro de una generación; no es la caché del B5); `CompiledConstraints.composition_id`/`constraint_hash` se calculan bajo demanda; `composition_hash` serializa directamente los AssetID (mismos bytes que `canonical_json`); `SolutionValidator._bounds` sin bucle por activo | La búsqueda compila y evalúa miles de composiciones y publica solo las finalistas; resultados idénticos (`REMEDIATION_BLOCK_3.md`) |
| 16 | `ConstraintHash` (F-5 de `AUDIT_BLOCK_3_CLOSURE.md`) | El `ConstraintHash` de un problema **cambia respecto a `block2-validated`** porque el `ConstraintSet` incorpora la política de banderas desconocidas y la configuración ADV/NAV, y las cotas compiladas incorporan E-10/E-11: el hash identifica el problema **matemático** resuelto y, si la semántica del problema cambió (nuevas restricciones o datos relevantes), no debe coincidir con el histórico. Los resultados económicos de la frontera continua se verificaron idénticos bit a bit | Forzar la igualdad con el hash histórico haría que dos problemas distintos compartieran identidad; las claves de caché del B5 parten de esta versión |
| 17 | Limitaciones residuales E-10 (F-7, F-8) | E-10 fija el límite inferior en 0 y prevalece sobre `MinWeight` y `min_holding_weight` (A-08), como `HOLD_OR_REDUCE` (documentado en la enmienda); los mensajes de factibilidad muestran floats de Python (corregido el cosmético `np.float64`) | Comportamiento decidido; no requiere corrección |

Caché: no existe caché de resultados en el Bloque 3 (`CCH-*` son del B5). `CompositionFactory` memoiza datos
derivados (restricciones compiladas y submatrices) **dentro de una búsqueda**, con una clave que es solo
...
| F3-02 | Normalización reversible | `BuiltProblem.normalize_return_row(escala)` → `NormalizedReturnProblem`: resta a la fila de retorno el múltiplo `λ = rᵀe/eᵀe` de la fila de presupuesto `e` (igualdad `eᵀx = presupuesto`) y desplaza la cota en `λ·presupuesto`; después multiplica fila y cota por `s = escala / máx\|coef.\|`, con el máximo tomado sobre los **pesos** y, si el bloque de pesos centrado es despreciable (`<= √eps` del máximo de toda la fila: retornos idénticos o casi idénticos), sobre **toda** la fila (pesos y variables de compra/venta; revisión post-merge P2-01). Es una transformación **exacta** del conjunto factible: mismas variables (los pesos se usan tal cual), mismo objetivo, mismas demás filas | `tests/unit/optimizers/test_return_row_normalization.py` (`fila'·x − cota' = s·(fila·x − cota)` para todo `x` con `1ᵀw = 1`). Retornos idénticos en `GROSS`, o `NET` sin costes (nada que escalar) → `None`; en `NET` con costes la fila conserva `−c_b/H`, `−c_s/H` y se normaliza (`tests/unit/frontiers/test_f3_post_merge_review.py`) |
...
| F3-06 | Diagnóstico | `SolveResult.recovery: RecoveryTrace \| None` (`models/solution.py`): estado y estado nativo del solver inicial, veredicto del LP, **todos** los intentos (`RecoveryAttempt`: estrategia, solver, estado, iteraciones, residuales primal/dual, escala, rechazo del validador) y resultado (`RecoveryOutcome`: `RECOVERED`, `CONFIRMED_INFEASIBLE`, `NOT_RECOVERED`). `SolveResult.iterations` y los tiempos suman **todos** los intentos, oráculo de HiGHS incluido (revisión post-merge P2-02; antes el oráculo se excluía de las iteraciones pero no de `solve_time`); `SolveResult.iterations_by_solver`, `FrontierSession.iterations_by_solver` y `FrontierDiagnostics.iterations_by_solver` desglosan por solver (OSQP, HiGHS). Sumar solvers distintos es una métrica agregada de actividad, no de esfuerzo computacional homogéneo | Los backends y `outputs/` no cambian |
...
- `candidates` depende de `constraints`, `validation`, `costs`, `models`, `config` y `parallel` (semillas), **no** de `optimizers` ni de `frontiers`: el evaluador exacto por QP pequeño es un `CompositionEvaluator` inyectado (`frontiers/candidate_evaluator.py`, ver §3.4 #3). `frontiers` depende de `candidates` y `parallel` (frontera global).
...
> **Implementado (Bloque 3).** El contexto real es `CandidateContext(portfolio_id, scenario_id, risk_model, universe, state, spec, costs, snapshot=None)`; además de `generate_candidate_compositions` existe `CandidateEngine.search(...) -> CandidateSearchResult` (composiciones + `CandidateDiagnostics`). Las desviaciones respecto a esta firma orientativa constan en §3.4.
...
| A-11 | §12 LiquidityConstraint | No definida y requiere NAV (no está en entradas). | MEDIA | `LiquidityCapacity_i = max_adv_participation · ADV_i · liquidation_days / NAV`. Posición nueva `w ≤ capacidad`; posición existente `w ≤ max(w_current, capacidad)` (protegida: no se fuerza a vender, no puede crecer por encima del peso actual). `[constraints.liquidity]` (`enabled`, `max_adv_participation` ∈ (0,1], `liquidation_days` ≥ 1, `fx_rates` explícitos) sin valores por defecto; con `enabled = true` faltan/inválidos parámetros o datos (`NAV` > 0, `ADV` ≥ 0) ⇒ error, nunca se desactiva en silencio. `ADV` y `NAV` en la misma divisa o FX explícito y trazable (par exacto; sin inventar ni invertir tipos). **Contrato de datos (cierre de la auditoría):** `ADV` = importe monetario medio diario (`AdvUnit.NOTIONAL_PER_DAY`, columna `ADVUnit`); `SHARES_PER_DAY`/`CONTRACTS_PER_DAY` se rechazan sin conversión (requeriría precio y política de valoración aprobados); unidad ausente o desconocida ⇒ error; `ADVCurrency`, `NAVCurrency` y `ADVSource` obligatorias (ausentes ⇒ error, nunca se supone igualdad); FX = unidades de NAVCurrency por unidad de ADVCurrency (`ADV_en_NAV = ADV · FX`, par exacto, sin inversión). Los datos se exigen a todos los activos de la composición, también a los restringidos. Es un límite de tamaño de posición, no garantiza ejecutabilidad de ventas y es distinto de un futuro límite de volumen negociable por operación. Precedencia E-09 y E-10 conservada. Aclarado por el usuario en la remediación del B3 (`MASTER_SPEC` E-11). | 3 | ACEPTADA |
